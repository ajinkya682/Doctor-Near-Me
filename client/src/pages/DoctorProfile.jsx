...
import { ArrowLeft, Star, Calendar, Clock, Award, ShieldCheck, Heart } from "lucide-react";
...
export default function DoctorProfile() {
  ...
  // Generate real dynamic dates (Today + next 7 days)
    ...
    <div className="flex flex-col min-h-full bg-white dark:bg-zinc-900 pb-10">
      ...
      <div className="px-6 mt-8 space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Availability</h3>
          ...
          {nextDays.map((date, index) => {
            ...
            return (
              <motion.div 
                ...
                className={`shrink-0 w-14 h-20 rounded-2xl flex flex-col items-center justify-center space-y-1 cursor-pointer transition-all ${isSelected ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/30' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-700'}`}
              >
      ...
      <div className="px-6 mt-6 pb-4">
        <button 
          onClick={() => alert(`Booking for ${format(nextDays[selectedDate], "MMMM d")}`)}
